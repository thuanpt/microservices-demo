...
// Tạo đơn hàng mới
func CreateOrder(db *sql.DB, eventSvc *service.EventService) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if eventSvc != nil {
			for _, item := range req.Items {
				// Lấy lại giá sản phẩm từ orderItems đã tính ở trên
				var price float64 = 0
				...
				_ = eventSvc.PublishOrderCreatedEvent(orderID, req.UserID, item.ProductID, item.Quantity, price*float64(item.Quantity))
			}
		}
