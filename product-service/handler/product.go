...
// Tạo product mới
func CreateProduct(db *sql.DB, eventSvc *service.EventService) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		// Publish event
		if eventSvc != nil {
			_ = eventSvc.PublishProductCreatedEvent(product.ID, product.Name, product.Price, product.Stock)
		}
...
func UpdateProductStock(db *sql.DB, eventSvc *service.EventService) gin.HandlerFunc {
	return func(c *gin.Context) {
		idStr := c.Param("id")
		id, err := strconv.Atoi(idStr)
		if err != nil {
		...
		// Publish event
		if eventSvc != nil {
			_ = eventSvc.PublishProductUpdatedEvent(id, "", 0, req.Stock)
		}
